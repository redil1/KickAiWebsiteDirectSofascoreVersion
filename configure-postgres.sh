...
create_user_and_database() {
    ...
    # Create user with retry - NO SUPERUSER for security
    while [ $attempt -le $max_attempts ]; do
        if su-exec postgres psql -c "CREATE USER kickai WITH NOSUPERUSER CREATEDB PASSWORD '${DB_PASSWORD:-kickai}';" >/dev/null 2>&1; then
            log_success "User kickai created successfully (non-superuser)"
            break
            ...
            su-exec postgres psql -c "ALTER USER kickai WITH NOSUPERUSER CREATEDB;" >/dev/null 2>&1 || true
            break
        else
...
if [ -f "/app/init.sql" ]; then
    ...
    log "Fixing permissions for kickai user..."
    su-exec postgres psql -d kickai_matches -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO kickai;" || true
    su-exec postgres psql -d kickai_matches -c "GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO kickai;" || true
    su-exec postgres psql -d kickai_matches -c "ALTER SCHEMA public OWNER TO kickai;" || true
fi
